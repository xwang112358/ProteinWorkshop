"""
SCOP (Structural Classification of Proteins) fold architecture dataset from ProteinShake.
...
class SCOPPSDataModule(ProteinDataModule):
    """
    DataModule for SCOP fold architecture classification from ProteinShake.

    Supports multiple split types (random, structure) while sharing
    PDB files and labels.

    Args:
        path: Root path to dataset (e.g., data/scop_proteinshake/)
    ...
    def __init__(
        ...
        logger.info(
            f"Setting up SCOP ProteinShake dataset with {split_type} split. "
...
if __name__ == "__main__":
    ...
    # Test random split
    print("=" * 70)
    print("Testing SCOP ProteinShake DataModule - Random Split")
    print("=" * 70)

    cfg_random = omegaconf.OmegaConf.load(
        constants.SRC_PATH / "config" / "dataset" / "scop_random.yaml"
    )
    cfg_random.datamodule.path = pathlib.Path(constants.DATA_PATH) / "scop_proteinshake"
